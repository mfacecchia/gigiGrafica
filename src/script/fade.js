...
setTimeout(() => {
    ...
    document.querySelectorAll('.fade:not(.active)').forEach(element => {
        ...
        if(fadeInElements.length >= 2 && fadeInElements.at(-1).yAxisOffset < fadeInElements.at(-2).yAxisOffset) needsSorting = true;
    });
    /*
...
function sortElementsPositions(elementsList){
    ...
    if(elementsList.length === 2) return elementsList[0].yAxisOffset > elementsList[1].yAxisOffset? [elementsList[1], elementsList[0]]: elementsList;
...
function merge(left, right){
    ...
    while(left.length && right.length){
        if(left[0].yAxisOffset > right[0].yAxisOffset) sorted.push(right.shift());
